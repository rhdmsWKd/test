...
function printContacts(contacts) {
  ...
  for(c of contacts) {
    const html = `
      <tr>
        <td>${c.no}</td>
        <td><a href='readtest.html?no=${c.no}'>${c.name}</a></td>
        <td>${c.tel}</td>
        <td>${c.address}</td>
      </tr>
    `;
...
function printPagination({prev, start, end, next, pageno}) {
  ...
  if(prev>0) {
    const html =`
      <li class='page-item'>
        <a class='page-link' href='test.html?pageno=${prev}'>이전으로</a>
      </li>`;
    $parent.append(html);
  }
  for(let i=start; i<=end; i++) {
    ...
    const html =`
      <li class='${className}'>
        <a class='page-link' href='test.html?pageno=${i}'>${i}</a>
      </li>`;
    $parent.append(html);  
  }
  if(next>0) {
    const html =`
      <li class='page-item'>
        <a class='page-link' href='test.html?pageno=${next}'>다음으로</a>
      </li>`;
    $parent.append(html);
  }
}
